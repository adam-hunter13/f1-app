...
import { Race, RaceResult } from "@/lib/api";
import { PageHeader, Card, SectionHeading, PosBadge, PointsChip } from "@/components/ui";

const SEASONS = Array.from({ length: 11 }, (_, i) => (2026 - i).toString());
...
export default function ResultsClient() {
  ...
  const [view, setView] = useState<"season" | "race">("season");

  // Load season winners list
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.jolpi.ca/ergast/f1/${season}/${selectedRound}/results.json`)
      ...
      .then((d) => {
        ...
        setRaceDetail(race ?? null);
        setView("race");
      })
      .finally(() => setLoadingDetail(false));
  }, [selectedRound, season]);

  const handleRaceSelect = (round: string) => {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Results"
        subtitle="Race by race — every winner, every point"
      />
      ...
      <div className="max-w-7xl mx-auto px-4 pb-12">
        ...
        {/* Back button when viewing a race */}
        {view === "race" && (
          <button
            onClick={handleBack}
        ...
        {view === "season" && (
          <>
            <SectionHeading>{season} Season Results</SectionHeading>
            {loadingRaces ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {races.map((race) => {
                  ...
                  return (
                    <button
                      key={race.round}
                      onClick={() => handleRaceSelect(race.round)}
                      className="text-left"
                    >
                      <Card className="p-4 card-hover h-full group cursor-pointer"
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <span
                            className="font-mono-f1 text-xs tracking-widest uppercase px-2 py-0.5 rounded"
                        ...
                        <p className="font-display text-base font-800 italic uppercase leading-tight mb-1 group-hover:text-opacity-80"
                        ...
                        <div className="mt-2 flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                          <span className="text-xs font-mono-f1 tracking-widest uppercase" style={{ color: team.secondary }}>
                            View Results →
                          </span>
                        </div>
                      </Card>
          ...
          <>
            {loadingDetail ? (
              ...
              <>
                ...
                <Card className="p-6 mb-6 relative overflow-hidden"
                  ...
                  <div className="relative">
                    <p className="font-mono-f1 text-xs tracking-widest uppercase mb-1" style={{ color: team.secondary }}>
                      Round {raceDetail.round} · {season}
                    </p>
                    <h2 className="font-display text-4xl font-900 italic uppercase">{raceDetail.raceName}</h2>
                    <p className="text-white/50 mt-1">
                      {countryFlags[raceDetail.Circuit.Location.country] || "🏁"}{" "}
                      {raceDetail.Circuit.circuitName} · {raceDetail.Circuit.Location.locality},{" "}
                      {raceDetail.Circuit.Location.country}
                    </p>
                    <p className="text-white/30 text-sm mt-1 font-mono-f1">
                      {new Date(raceDetail.date).toLocaleDateString("en-US", {
                ...
                <div className="space-y-2">
                  {(raceDetail.Results ?? []).map((result, i) => {
                    ...
                    return (
                      <Card
                        key={result.position}
                        className={`px-4 py-3 card-hover flex items-center gap-4 ${i < 3 ? "border-opacity-30" : ""}`}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {isFastestLap && (
                              <span
                                className="text-xs px-1.5 py-0.5 rounded font-mono-f1 flex-shrink-0"
                ...
                {/* Legend */}
                <div className="mt-4 flex items-center gap-4 text-xs text-white/30 font-mono-f1">
                  <span>⚡ <span style={{ color: "#9B59B6" }}>FL</span> = Fastest Lap bonus point</span>
                </div>
              </>
            ) : (
              <Card className="p-12 text-center">
